	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				levelnoton: '❬ X ❭ *Leveling n ativado*',
				...
				only: {
					...
					ownerB: '[❗] Este comando só pode ser usado pelo sebola! ❌',
			...
			const NomerOwner = '5531983941907@s.whatsapp.net'
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'animet':
						...
						nimek =  n[Math.floor(Math.random() * n.length)];
						pok = await getBuffer(nimek)
						client.sendMessage(from, pok, image, { quoted: mek, caption: `🤖* : *${body.slice(11)}*`})
						break
				    case 'randomanime':
				...
				client.updatePresence(from, Presence.composing) 
						   ...
						   if (text.includes("random"))
						   {
							...
							axios.get(url)
							  .then((result) => {
								...
								.then(
									(response) => {
							conn.sendMessage(id, 'Jaja eu mando', MessageType.text, { quoted: m } )
					...
					mentions('╔══✪〘 Mencionando Todo Mundo! 〙✪══\n╠➥'+teks+'╚═〘 sebola.dll〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todo Mundo! 〙✪══\n╠➥'+teks+'╚═〘 DLL 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
